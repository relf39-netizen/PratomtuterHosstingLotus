...
import TeacherManager from './teacher/TeacherManager'; 
import ProfileManager from './teacher/ProfileManager';
import ClassroomManager from './teacher/ClassroomManager';
import ExecutiveDashboard from './teacher/ExecutiveDashboard';
...
const TeacherDashboard: React.FC<TeacherDashboardProps> = ({ teacher: initialTeacher, onLogout, initialLogo, appName, onTeacherUpdate }) => {
  ...
  // สิทธิ์การเข้าถึงหน้า NT/O-NET (ให้สิทธิ์ผู้บริหารด้วย)
    ...
    <div className="max-w-7xl mx-auto pb-20 relative font-prompt">
        <div className="bg-white rounded-3xl shadow-sm border border-slate-200 p-6 mb-8 flex flex-col md:flex-row justify-between items-center gap-4">
            <div className="flex items-center gap-4">
                ...
                <div>
                    <h2 className="text-2xl font-bold text-slate-800 flex items-center gap-2"> ห้องพักครู {finalAppName}</h2>
        ...
        {activeTab !== 'menu' && (
            <div className="bg-white rounded-[40px] p-6 md:p-8 shadow-sm border border-slate-200 min-h-[600px]">
                <button onClick={() => { setActiveTab('menu'); setOnetLevel(null); }} className="mb-8 flex items-center gap-2 text-slate-500 hover:text-indigo-600 transition font-black text-sm uppercase tracking-widest group">
                ...
                {activeTab === 'students' && <StudentManager students={students} teacher={teacher} canManageAll={!!canManageAll} schoolSettings={schoolInfo} isDirector={!!isExecutive} onRefresh={loadData} />}
                {/* Fix: Changed loadInitialData to loadData */}
                {activeTab === 'subjects' && <SubjectManager subjects={availableSubjects} teacher={teacher} canManageAll={canManageAll} myGrades={GRADES} onRefresh={loadData}/>}
                ...
                {activeTab === 'school-settings' && (
                    <div className="animate-fade-in max-w-6xl mx-auto">
                        ...
                        {settingsTab === 'students' ? (
                            /* Fix: Changed loadInitialData to loadData */
                            <StudentManager students={students} teacher={teacher} canManageAll={!!isAdminUser} schoolSettings={schoolInfo} isDirector={!!isExecutive} onRefresh={loadData} />
                        ...
                        {onetLevel && (
                            <div className="flex flex-wrap gap-2 mb-8 bg-slate-100 p-1.5 rounded-[22px] w-fit shadow-inner">
                                <button 
                                    ...
                                    className={`px-6 py-2 rounded-xl text-xs font-black transition-all flex items-center gap-2 ${onetLevel === 'P3' ? 'bg-white text-emerald-600 shadow-sm' : 'text-slate-600 hover:text-slate-800'}`}
                                >
                                ...
                                <button 
                                    ...
                                    className={`px-6 py-2 rounded-xl text-xs font-black transition-all flex items-center gap-2 ${onetLevel === 'P6' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-600 hover:text-slate-800'}`}
                                >
                                ...
                                <button 
                                    ...
                                    className={`px-6 py-2 rounded-xl text-xs font-black transition-all flex items-center gap-2 ${onetLevel === 'M3' ? 'bg-white text-purple-600 shadow-sm' : 'text-slate-600 hover:text-slate-800'}`}
                                >

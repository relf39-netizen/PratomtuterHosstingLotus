...
function normalizeExamResultRow(row) {
    if (!row)
        return row;
...
async function fetchSupabaseTableAllRows(supabaseUrl, supabaseKey, table) {
    let allRows = [];
    let offset = 0;
    const limit = 1000;
    let hasMore = true;
...
async function ensureSuperAdminsExist() {
    try {
        if (pool) {
            try {
                const checkTable = await query("SHOW TABLES LIKE 'teachers'");
                if (checkTable && checkTable.length > 0) {
                    const teachers = await query("SELECT * FROM `teachers` WHERE `role` = 'SUPER_ADMIN'");
                    const hasAdmin = teachers.some((t) => t.username === 'admin');
                    const hasPeyarm = teachers.some((t) => t.username === 'peyarm');
                    if (!hasAdmin) {
        ...
        try {
            ...
            const hasAdminJson = db.teachers.some((t) => t.username === 'admin');
            const hasPeyarmJson = db.teachers.some((t) => t.username === 'peyarm');
            let updated = false;
            if (!hasAdminJson) {
        ...
        if (pool) {
            switch (action) {
                    ...
                    try {
                        if (operation === 'select') {
                            ...
                            if (activeFilters.length > 0) {
                                const whereParts = activeFilters.map((f) => `\`${f.field}\` = ?`);
                                sql += ` WHERE ${whereParts.join(' AND ')}`;
                                params.push(...activeFilters.map((f) => f.value));
                            }
                            if (orderCol) {
                ...
                case 'updateAppSettings': {
                    ...
                    await query('INSERT INTO app_settings (id, logo_url, app_name) VALUES (1, ?, ?) ON DUPLICATE KEY UPDATE logo_url = ?, app_name = ?', [logo_url || null, app_name || null, logo_url || null, app_name || null]);
                    return res.json({ success: true });
                }
                case 'fetchAppData': {
                ...
                case 'getTeacherDashboard': {
                    ...
                    const results = await query('SELECT * FROM exam_results WHERE LOWER(TRIM(school)) = LOWER(TRIM(?)) OR student_id IN (SELECT id FROM students WHERE LOWER(TRIM(school)) = LOWER(TRIM(?))) OR assignment_id IN (SELECT id FROM assignments WHERE LOWER(TRIM(school)) = LOWER(TRIM(?))) ORDER BY timestamp DESC', [cleanSchool, cleanSchool, cleanSchool]);
                ...
                case 'saveScore': {
                    ...
                    let updated = false;
                    if (assignmentId) {
                        const existingRows = await query('SELECT id, details FROM exam_results WHERE student_id = ? AND assignment_id = ? LIMIT 1', [studentId, assignmentId]);
                        if (existingRows && existingRows.length > 0) {
                            ...
                            if (isRetake || (details && details.isRetake)) {
                                ...
                                await query('UPDATE exam_results SET details = ? WHERE id = ?', [JSON.stringify(retakeData), existingRows[0].id]);
                                updated = true;
                            }
                            else {
                                await query('UPDATE exam_results SET score = ?, total_questions = ?, subject = ?, category = ?, timestamp = ?, details = ? WHERE id = ?', [score, total, subject, category, Date.now(), details ? JSON.stringify(details) : null, existingRows[0].id]);
                                updated = true;
                            }
                        }
                    }
                    if (!updated) {
                        ...
                        await query('INSERT INTO exam_results (student_id, student_name, school, score, total_questions, subject, assignment_id, category, timestamp, details) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', [studentId, studentName, school, score, total, subject, assignmentId || null, category, Date.now(), finalDetails ? JSON.stringify(finalDetails) : null]);
                    }
                    if (earnedStars > 0) {
                ...
                case 'toggleRetakePermission': {
                    ...
                    if (mode === 'all' && assignmentId) {
                        ...
                        const rows = await query('SELECT id, details FROM exam_results WHERE LOWER(TRIM(assignment_id)) = LOWER(TRIM(?)) OR LOWER(TRIM(assignment_id)) = LOWER(TRIM(?))', [cleanAsgId, cleanAsgId]);
                        for (const r of rows || []) {
                        ...
                        if (allowRetake) {
                            const asgRows = await query('SELECT subject, category, school, grade, target_classrooms FROM assignments WHERE LOWER(TRIM(id)) = LOWER(TRIM(?)) LIMIT 1', [cleanAsgId]);
                            ...
                            const eligibleStRows = (stRows || []).filter((st) => {
                                if (asgGrade && asgGrade !== 'ALL' && String(st.grade || '').trim() !== String(asgGrade).trim())
                                    return false;
                                if (targetRooms && targetRooms.length > 0 && st.classroom && !targetRooms.includes(String(st.classroom).trim()))
                                    return false;
                                return true;
                            });
                            for (const st of eligibleStRows) {
                                const existing = await query('SELECT id FROM exam_results WHERE LOWER(TRIM(student_id)) = LOWER(TRIM(?)) AND LOWER(TRIM(assignment_id)) = LOWER(TRIM(?)) LIMIT 1', [String(st.id).trim(), cleanAsgId]);
                                if (!existing || existing.length === 0) {
                                    const detailsObj = { retakeAllowed: true };
                                    await query('INSERT INTO exam_results (student_id, student_name, school, assignment_id, subject, category, score, total_questions, timestamp, details) VALUES (?, ?, ?, ?, ?, ?, 0, 0, ?, ?)', [String(st.id), st.name || '', st.school || schoolName || '', cleanAsgId, asg.subject || '', asg.category || 'GENERAL', Date.now(), JSON.stringify(detailsObj)]);
                                }
                            }
                        }
                        return res.json({ success: true });
                    ...
                    if (studentId && assignmentId) {
                        const rowsByStudAsg = await query('SELECT * FROM exam_results WHERE LOWER(TRIM(student_id)) = LOWER(TRIM(?)) AND LOWER(TRIM(assignment_id)) = LOWER(TRIM(?))', [String(studentId).trim(), String(assignmentId).trim()]);
                    ...
                    if (rowsToUpdate.length > 0) {
                        ...
                        await query('INSERT INTO exam_results (student_id, student_name, school, assignment_id, subject, category, score, total_questions, timestamp, details) VALUES (?, ?, ?, ?, ?, ?, 0, 0, ?, ?)', [String(studentId), studentName, finalSchool, String(assignmentId), subject, category, Date.now(), JSON.stringify(detailsObj)]);
                        return res.json({ success: true });
                    }
                ...
                case 'addAssignment': {
                    ...
                    await query('INSERT INTO assignments (id, school, subject, grade, question_count, deadline, created_by, title, target_classrooms, target_classroom_ids, category, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', [id, school, subject, grade, questionCount, deadline, createdBy, title || '', JSON.stringify(targetClassrooms || []), JSON.stringify(targetClassroomIds || []), category, status || 'OPEN']);
                ...
                case 'addQuestion': {
                    ...
                    await query('INSERT INTO questions (id, subject, grade, text, image, choices, correct_choice_id, explanation, school, teacher_id, assignment_id, target_classrooms, target_classroom_ids, unit) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', [
                ...
                case 'editQuestion': {
                    ...
                    await query('UPDATE questions SET subject = ?, grade = ?, text = ?, image = ?, choices = ?, correct_choice_id = ?, explanation = ?, target_classrooms = ?, target_classroom_ids = ?, unit = ? WHERE id = ?', [
                ...
                case 'addSubject': {
                    ...
                    await query('INSERT INTO subjects (id, name, school, teacher_id, grade, target_classrooms, target_classroom_ids, icon, color) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', [id, sub.name, school, sub.teacherId, sub.grade, JSON.stringify(sub.targetClassrooms || []), JSON.stringify(sub.targetClassroomIds || []), sub.icon || '', sub.color || '']);
                    return res.json({ success: true, id });
                }
                case 'deleteSubject': {
                    ...
                    try {
                        ...
                        if (subRows && subRows.length > 0) {
                            ...
                            if (subName) {
                                ...
                                const asgIds = (asgRows || []).map((a) => a.id).filter(Boolean);
                                if (asgIds.length > 0) {
                ...
                case 'requestRegistration': {
                    const { payload } = args;
                    await query('INSERT INTO registration_requests (citizen_id, name, surname, school_id, school_name, school_code, position, type, timestamp, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', [payload.citizenId, payload.name, payload.surname, payload.schoolId, payload.schoolName || '', payload.schoolCode || '', payload.position || '', payload.type, Date.now(), 'pending']);
                    return res.json({ success: true });
                }
                case 'approveRegistration': {
                    ...
                    if (req.type === 'SCHOOL') {
                        ...
                        await query('INSERT INTO teachers (id, username, password, name, school, citizen_id, role, status, position, grade_level) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', [generateId('tea_'), req.citizenId, '123456', `${req.name} ${req.surname}`, req.schoolName, req.citizenId, 'SCHOOL_ADMIN', 'active', req.position, 'ALL']);
                    }
                    else {
                        await query('INSERT INTO teachers (id, username, password, name, school, citizen_id, role, status, position, grade_level) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', [generateId('tea_'), req.citizenId, '123456', `${req.name} ${req.surname}`, schoolName, req.citizenId, role, 'active', req.position, grade]);
                    }
                ...
                case 'getSchoolPendingRegistrations': {
                    ...
                    if (cleanSchoolName) {
                        ...
                        if (schoolRows && schoolRows.length > 0) {
                            if (!targetSchoolId && String(schoolRows[0].id) !== 'NEW_SCHOOL')
                                targetSchoolId = String(schoolRows[0].id);
                            if (schoolRows[0].school_code && String(schoolRows[0].school_code).trim() !== 'NEW_SCHOOL')
                                schoolCode = String(schoolRows[0].school_code).trim();
                        }
                    }
                    const conds = [];
                    const params = [];
                ...
                case 'manageStudent': {
                    ...
                    if (type === 'add') {
                        ...
                        await query('INSERT INTO students (id, name, username, password, school, avatar, grade, classroom, stars) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', [id, payload.name, payload.username, payload.password, payload.school, payload.avatar || '👶', payload.grade, payload.classroom, 0]);
                        return res.json({ success: true, student: { id, ...payload, stars: 0 } });
                    }
                    if (type === 'edit') {
                        await query('UPDATE students SET name = ?, username = ?, password = COALESCE(NULLIF(?, ""), password), avatar = ?, grade = ?, classroom = ? WHERE id = ?', [payload.name, payload.username, payload.password || '', payload.avatar || '👶', payload.grade, payload.classroom, payload.id]);
                        return res.json({ success: true });
                    }
                    if (type === 'delete') {
                ...
                case 'manageClassroom': {
                    ...
                    if (type === 'add') {
                        ...
                        await query('INSERT INTO classrooms (school, grade_level, room_number, name) VALUES (?, ?, ?, ?)', [payload.school, payload.gradeLevel, payload.roomNumber, name]);
                        return res.json({ success: true });
                    }
                    if (type === 'delete') {
                ...
                case 'cleanupOrphanedSubjects': {
                    try {
                        ...
                        const validSubjectNamesGlobally = new Set();
                ...
                case 'repairDatabaseStructure': {
                    try {
                        ...
                        (allTeachers || []).forEach((t) => {
                            ...
                            if (!teachersBySchool.has(sch))
                                teachersBySchool.set(sch, []);
                        ...
                        for (const sub of (allSubjects || [])) {
                            ...
                            if (sch && nm)
                                subjectByKey.set(`${sch}::${nm}`, sub);
                            if (nm && !subjectByKey.has(`global::${nm}`))
                                subjectByKey.set(`global::${nm}`, sub);
                            if (!sub.teacher_id || !teacherById.has(String(sub.teacher_id))) {
                                const schoolTeachers = teachersBySchool.get(sch) || [];
                                const matchedTeacher = schoolTeachers.find((t) => t.role === 'SCHOOL_ADMIN' || t.role === 'SUPER_ADMIN') || schoolTeachers[0];
                                if (matchedTeacher) {
                        ...
                        for (const asg of (allAssignments || [])) {
                            ...
                            if (!asg.teacher_id) {
                                ...
                                const schoolTeachers = teachersBySchool.get(sch) || [];
                                const matchedTeacher = schoolTeachers.find((t) => t.name === creator || t.username === creator) || schoolTeachers[0];
                                if (matchedTeacher) {
                ...
                case 'importFromSupabase': {
                    ...
                    for (const table of tablesToImport) {
                        try {
                            ...
                            const allowedCols = cols.map((c) => (c.Field || c.field || '').toString()).filter(Boolean);
        ...
        switch (action) {
            case 'supabaseQuery': {
                ...
                if (operation === 'select') {
                    ...
                    if (activeFilters.length > 0) {
                        rows = rows.filter((r) => {
                            return activeFilters.every((f) => String(r[f.field] ?? '') === String(f.value ?? ''));
                        });
                    }
                    if (orderCol) {
                        rows = [...rows].sort((a, b) => String(a[orderCol] || '').localeCompare(String(b[orderCol] || '')));
                    }
                    if (limitNum) {
                ...
                if (operation === 'delete') {
                    if (filterField) {
                        db[table] = db[table].filter((r) => String(r[filterField] || '') !== String(filterVal));
                        saveJsonDb(db);
                    }
                    return res.json({ success: true });
                }
            ...
            case 'getDataForStudent': {
                ...
                const results = db.exam_results.filter((r) => String(r.student_id) === String(student.id))
                    .sort((a, b) => b.timestamp - a.timestamp);
                const assignments = db.assignments.filter((a) => String(a.school).trim().toLowerCase() === cleanSchool);
                const subjects = db.subjects.filter((s) => String(s.school).trim().toLowerCase() === cleanSchool);
                const questions = db.questions.filter((q) => String(q.school).trim().toLowerCase() === cleanSchool);
                return res.json({ results, assignments, subjects, questions });
            }
            case 'getTeacherDashboard': {
                const { school } = args;
                ...
                const studentIds = new Set((db.students || [])
                    .filter((s) => String(s.school || '').trim().toLowerCase() === cleanSchool)
                    .map((s) => String(s.id)));
                const assignmentIds = new Set((db.assignments || [])
                    .filter((a) => String(a.school || '').trim().toLowerCase() === cleanSchool)
                    .map((a) => String(a.id)));
                const students = (db.students || []).filter((s) => String(s.school || '').trim().toLowerCase() === cleanSchool);
                const results = (db.exam_results || []).filter((r) => {
                ...
                const assignments = (db.assignments || []).filter((a) => String(a.school || '').trim().toLowerCase() === cleanSchool);
                const subjects = (db.subjects || []).filter((s) => String(s.school || '').trim().toLowerCase() === cleanSchool);
                const schoolInfo = (db.schools || []).find((s) => String(s.name || '').trim().toLowerCase() === cleanSchool) || null;
            ...
            case 'saveScore': {
                ...
                let existingIndex = -1;
                if (assignmentId) {
                    existingIndex = db.exam_results.findIndex((r) => String(r.student_id) === String(studentId) && String(r.assignment_id) === String(assignmentId));
                }
                if (existingIndex >= 0) {
            ...
            case 'toggleRetakePermission': {
                ...
                if (mode === 'all' && assignmentId) {
                    ...
                    if (allowRetake) {
                        const asg = (db.assignments || []).find((a) => String(a.id) === String(assignmentId));
                        const schoolName = asg?.school || '';
                        const eligibleStudents = (db.students || []).filter((s) => (!schoolName || s.school === schoolName) &&
                        ...
                        eligibleStudents.forEach((st) => {
                            const existing = (db.exam_results || []).find((r) => String(r.student_id || r.studentId) === String(st.id) && String(r.assignment_id || r.assignmentId) === String(assignmentId));
                            if (!existing) {
                                if (!db.exam_results)
                                    db.exam_results = [];
                ...
                const matchingRows = (db.exam_results || []).filter((item) => {
                    if (resultId && !String(resultId).startsWith('res_temp_') && String(item.id) === String(resultId))
                        return true;
                    if (studentId && assignmentId && String(item.student_id || item.studentId) === String(studentId) && String(item.assignment_id || item.assignmentId) === String(assignmentId))
                        return true;
                    return false;
                });
                if (matchingRows.length > 0) {
                    matchingRows.forEach((rItem) => {
            ...
            case 'verifyStudentLogin': {
                ...
                let student;
                if (password) {
                    student = db.students.find((s) => s.username === username && s.password === password);
                }
                else {
                    // legacy PIN
                    student = db.students.find((s) => s.id === username);
                }
                if (student) {
            ...
            case 'toggleAssignmentStatus': {
                const { id, currentStatus } = args;
                const newStatus = currentStatus === 'LOCKED' ? 'OPEN' : 'LOCKED';
                const asg = db.assignments.find((a) => String(a.id) === String(id));
                if (asg) {
                    let title = asg.title || '';
                    title = title.replace('::LOCKED', '').replace('::OPEN', '').trim();
                    if (newStatus === 'LOCKED')
            ...
            case 'deleteAssignment': {
                ...
                db.questions = (db.questions || []).filter((q) => String(q.assignment_id || q.assignmentId || '') !== targetId);
                db.exam_results = (db.exam_results || []).filter((r) => String(r.assignmentId || r.assignment_id || '') !== targetId);
                db.assignments = (db.assignments || []).filter((a) => String(a.id) !== targetId);
                saveJsonDb(db);
                return res.json({ success: true });
            }
            case 'getQuestionsByAssignment': {
                const { assignmentId } = args;
                const rows = db.questions.filter((q) => String(q.assignment_id) === String(assignmentId));
                return res.json({ data: rows });
            }
            case 'addQuestion': {
                const { q } = args;
            ...
            case 'editQuestion': {
                ...
                const existingQ = db.questions.find((qq) => String(qq.id) === String(q.id));
                if (existingQ) {
            ...
            case 'getTeacherById': {
                const { id } = args;
                const teacher = db.teachers.find((t) => String(t.id) === String(id)) || null;
            ...
            case 'getSubjects': {
                const { school } = args;
                const rows = db.subjects.filter((s) => String(s.school).trim().toLowerCase() === String(school).trim().toLowerCase());
                return res.json({ data: rows });
            }
            case 'addSubject': {
                const { school, sub } = args;
            ...
            case 'deleteSubject': {
                const { school, id } = args;
                const cleanSchool = String(school || '').trim().toLowerCase();
                const targetSub = (db.subjects || []).find((s) => String(s.id) === String(id) && String(s.school || '').trim().toLowerCase() === cleanSchool);
                ...
                if (subName) {
                    ...
                    db.assignments = (db.assignments || []).filter((a) => {
                        ...
                        if (aSchool === cleanSchool && (aSub === subName || aSub.includes(subName) || subName.includes(aSub))) {
                            if (a.id)
                                deletedAssignmentIds.add(String(a.id));
                            return false;
                        }
                        return true;
                    });
                    ...
                    db.exam_results = (db.exam_results || []).filter((r) => {
                        const rSchool = String(r.school || '').trim().toLowerCase();
            ...
            case 'updateSchoolSettings': {
                const { schoolName, settings } = args;
                const school = db.schools.find((s) => String(s.name).trim().toLowerCase() === String(schoolName).trim().toLowerCase());
                if (school) {
            ...
            case 'manageTeacher': {
                const { type, payload } = args;
                if (type === 'edit') {
                    const t = db.teachers.find((x) => String(x.id) === String(payload.id));
                    if (t) {
                        t.name = payload.name;
                ...
                if (type === 'delete') {
                    db.teachers = db.teachers.filter((x) => String(x.id) !== String(payload.id));
                    saveJsonDb(db);
                    return res.json({ success: true });
                }
                return res.json({ success: false });
            ...
            case 'manageSchool': {
                ...
                if (type === 'update_status') {
                    const s = db.schools.find((x) => String(x.id) === String(payload.id));
                    if (s) {
                ...
                if (type === 'delete') {
                    db.schools = db.schools.filter((x) => String(x.id) !== String(payload.id));
                    saveJsonDb(db);
                    return res.json({ success: true });
                }
                return res.json({ success: false });
            ...
            case 'findSchoolByCode': {
                const { code } = args;
                const school = db.schools.find((s) => String(s.school_code).trim().toLowerCase() === String(code).trim().toLowerCase()) || null;
            ...
            case 'approveRegistration': {
                const { req, role, grade, schoolName } = args;
                if (req.type === 'SCHOOL') {
                    const existsSchool = db.schools.some((s) => s.name === req.schoolName);
                    if (!existsSchool) {
                ...
                const r = db.registration_requests.find((x) => String(x.id) === String(req.id));
                if (r)
                    r.status = 'approved';
                saveJsonDb(db);
                return res.json({ success: true });
            ...
            case 'rejectRegistration': {
                const { id } = args;
                const r = db.registration_requests.find((x) => String(x.id) === String(id));
                if (r)
                    r.status = 'rejected';
                saveJsonDb(db);
                return res.json({ success: true });
            ...
            case 'getAllPendingRegistrations': {
                const rows = db.registration_requests.filter((r) => r.status === 'pending');
                return res.json({ data: rows });
            }
            case 'getSchoolPendingRegistrations': {
                const { schoolName, schoolId } = args;
                const cleanSchoolName = String(schoolName || '').trim().toLowerCase();
                const targetSchool = (db.schools || []).find((s) => String(s.name || '').trim().toLowerCase() === cleanSchoolName);
                ...
                const rows = (db.registration_requests || []).filter((r) => {
                    if (String(r.status || '').toLowerCase() !== 'pending' || String(r.type || '').toUpperCase() !== 'TEACHER')
                        return false;
                    ...
                    if (cleanSchoolName && rSchoolName === cleanSchoolName)
                        return true;
                    if (targetSchoolId && targetSchoolId !== 'NEW_SCHOOL' && rSchoolId === targetSchoolId)
                        return true;
                    if (targetSchoolCode && targetSchoolCode !== 'NEW_SCHOOL' && rSchoolCode === targetSchoolCode)
                        return true;
                    return false;
            ...
            case 'manageStudent': {
                ...
                if (type === 'edit') {
                    const std = db.students.find((x) => String(x.id) === String(payload.id));
                    if (std) {
                        std.name = payload.name;
                        std.username = payload.username;
                        if (payload.password)
                            std.password = payload.password;
                ...
                if (type === 'delete') {
                    db.students = db.students.filter((x) => String(x.id) !== String(payload.id));
                    saveJsonDb(db);
                    return res.json({ success: true });
                }
                if (type === 'promote_bulk') {
                ...
                if (type === 'reset_scores') {
                    db.exam_results = db.exam_results.filter((r) => String(r.student_id) !== String(payload.id));
                    saveJsonDb(db);
                    return res.json({ success: true });
                }
                if (type === 'change_password') {
            ...
            case 'getClassrooms': {
                const { school } = args;
                const cleanSchool = String(school).trim().toLowerCase();
                const rows = db.classrooms.filter((c) => String(c.school).trim().toLowerCase() === cleanSchool)
            ...
            case 'manageClassroom': {
                ...
                if (type === 'delete') {
                    db.classrooms = db.classrooms.filter((c) => String(c.id) !== String(payload.id));
                    saveJsonDb(db);
                    return res.json({ success: true });
                }
                return res.json({ success: false });
            ...
            case 'deleteQuestion': {
                const { id } = args;
                db.questions = db.questions.filter((q) => String(q.id) !== String(id));
                saveJsonDb(db);
                return res.json({ success: true });
            }
            case 'getQuestionsBySubject': {
                const { subject } = args;
                const rows = db.questions.filter((q) => q.subject === subject);
                return res.json({ data: rows });
            }
            case 'getQuestionsBySubjectAndGrade': {
                const { subject, grade, school } = args;
                const rows = db.questions.filter((q) => {
                    ...
                    if (grade && grade !== 'ALL' && q.grade !== grade)
                        return false;
                    if (school && school !== 'ALL' && String(q.school || '').trim().toLowerCase() !== String(school || '').trim().toLowerCase())
                        return false;
                    return true;
                });
                return res.json({ data: rows });
            ...
            case 'getSchools': {
                const sorted = [...db.schools].sort((a, b) => a.name.localeCompare(b.name));
                return res.json({ data: sorted });
            }
            case 'getAllTeachers': {
                const sorted = [...db.teachers].sort((a, b) => a.name.localeCompare(b.name));
                return res.json({ data: sorted });
            }
            case 'getSuperAdminStats': {
                return res.json({
...
async function startServer() {
    const isInsideDist = path_1.default.basename(__dirname) === 'dist';
    let distPath = '';
    if (isInsideDist) {
        distPath = __dirname;
    }
    ...
    const hasDist = fs_1.default.existsSync(path_1.default.join(distPath, 'index.html'));
